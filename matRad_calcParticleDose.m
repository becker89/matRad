...
for i = 1:dij.numOfBeams % loop over all beams
    ...
    [radDepthV,radDepthCube,~] = matRad_rayTracing(stf(i),ct,V,rot_coordsV,lateralCutoffRayTracing);
    ...
    for j = 1:stf(i).numOfRays % loop over all rays
        ...
        if ~isempty(stf(i).ray(j).energy)
            ...
            sigmaIni = matRad_calcSigmaIni(machine.data,stf(i).ray(j));
...
%             finalWeight = reshape(finalWeight,[],1);
            ...
            for k = 1:stf(i).numOfBixelsPerRay(j) % loop over all bixels per ray
                ...
                % project coordinates to central ray - spot plane
                % maybe apply cutoff before
